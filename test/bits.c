...
Test(round_up_nearest_power_of_two, uint_num) {
    unsigned int x = 0;
    cr_assert(sdr_round_up_nearest_po2(x) == 1);
    x = 1;
    cr_assert(sdr_round_up_nearest_po2(x) == 1);
    x = 2;
    cr_assert(sdr_round_up_nearest_po2(x) == 2);
    x = 3;
    cr_assert(sdr_round_up_nearest_po2(x) == 4, "num: %d\n", sdr_round_up_nearest_po2(x));
    x = 4;
    cr_assert(sdr_round_up_nearest_po2(x) == 4);
    x = 5;
    cr_assert(sdr_round_up_nearest_po2(x) == 8);
    x = 6;
    cr_assert(sdr_round_up_nearest_po2(x) == 8);
    x = 7;
    cr_assert(sdr_round_up_nearest_po2(x) == 8);
    ...
    x = 15;
    cr_assert(sdr_round_up_nearest_po2(x) == 16);
    x = 16;
    cr_assert(sdr_round_up_nearest_po2(x) == 16);

    x = 1000;
    cr_assert(sdr_round_up_nearest_po2(x) == 1024);
    x = 1024;
    cr_assert(sdr_round_up_nearest_po2(x) == 1024);
}
